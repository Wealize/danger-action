...
describe("combineEntries()", () => {
    it("can add non-zero line counts", () => {
        ...
        const result = coverage_model_1.combineEntries(entry1, entry2);
...
describe("meetsThreshold()", () => {
    ...
    it("returns false when lines percentage below threshold", () => {
        const result = coverage_model_1.meetsThreshold(entry, { lines: 80, statements: 40, branches: 40, functions: 40 });
        expect(result).toBe(false);
    });
    it("returns false when statements percentage below threshold", () => {
        const result = coverage_model_1.meetsThreshold(entry, { lines: 40, statements: 80, branches: 40, functions: 40 });
        expect(result).toBe(false);
    });
    it("returns false when branches percentage above threshold", () => {
        const result = coverage_model_1.meetsThreshold(entry, { lines: 40, statements: 40, branches: 80, functions: 40 });
        expect(result).toBe(false);
    });
    it("returns false when functions percentage above threshold", () => {
        const result = coverage_model_1.meetsThreshold(entry, { lines: 40, statements: 40, branches: 40, functions: 80 });
        expect(result).toBe(false);
    });
    it("returns true when all coverage above thresholds", () => {
        const result = coverage_model_1.meetsThreshold(entry, { lines: 40, statements: 40, branches: 40, functions: 40 });
        expect(result).toBe(true);
    });
    it("returns true when all coverage equal to thresholds", () => {
        const result = coverage_model_1.meetsThreshold(entry, { lines: 50, statements: 50, branches: 50, functions: 50 });
        expect(result).toBe(true);
    });
});
describe("makeCoverageModel", () => {
    ...
    it("calculates the average of the final two elided entries", () => {
        const output = coverage_model_1.makeCoverageModel(2, files, coverage);
    ...
    it("doesn't elide when total number of files is equal to numberOfEntries to display", () => {
        const output = coverage_model_1.makeCoverageModel(4, files, coverage);
...
describe("sortFiles", () => {
    ...
    it("sorts files by their line coverage percentage in descending order", () => {
        const output = coverage_model_1.sortFiles(["file1", "file2", "file3"], coverage, "most-coverage");
        expect(output).toEqual(["file3", "file2", "file1"]);
    });
    it("sorts files by their line coverage percentage in ascending order", () => {
        const output = coverage_model_1.sortFiles(["file1", "file2", "file3"], coverage, "least-coverage");
        expect(output).toEqual(["file1", "file2", "file3"]);
    });
    it("skips files not in input file list", () => {
        const output = coverage_model_1.sortFiles(["file1", "file3"], coverage, "most-coverage");
        expect(output).toEqual(["file3", "file1"]);
    });
    it("sorts files by the number of lines in descending order", () => {
        const output = coverage_model_1.sortFiles(["file1", "file2", "file3"], coverage, "largest-file-size");
        expect(output).toEqual(["file2", "file3", "file1"]);
    });
    it("sorts files the number of lines in ascending order", () => {
        const output = coverage_model_1.sortFiles(["file1", "file2", "file3"], coverage, "smallest-file-size");
        expect(output).toEqual(["file1", "file3", "file2"]);
    });
    it("sorts files by the number of uncovered lines in descending order", () => {
        const output = coverage_model_1.sortFiles(["file1", "file2", "file3"], coverage, "uncovered-lines");
        expect(output).toEqual(["file3", "file2", "file1"]);
    });
    it("sorts files in alphabetical order", () => {
        const output = coverage_model_1.sortFiles(["c", "b", "a"], coverage, "alphabetically");
