 ...
 * @returns {Promise<fetch.Response>} network-y promise
 */
function api(url, init, suppressErrorReporting, provessEnv) {
    var _this = this;
    if (provessEnv === void 0) { provessEnv = process.env; }
    ...
    if (global.verbose && global.verbose === true) {
        ...
        var showToken = provessEnv["DANGER_VERBOSE_SHOW_TOKEN"];
        var token = provessEnv["DANGER_GITHUB_API_TOKEN"] || provessEnv["GITHUB_TOKEN"];
        if (init.headers) {
    ...
    var proxy = provessEnv["HTTPS_PROXY"] || provessEnv["https_proxy"] || provessEnv["HTTP_PROXY"] || provessEnv["http_proxy"];
    if (!agent && proxy) {
